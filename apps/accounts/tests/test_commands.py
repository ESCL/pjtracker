...
class CreteExampleAccountTest(TestCase):
    ...
    def test_create_example_account(self):
        ...
        self.assertEqual(User.objects.count(), n_users + 6)

        # Check their data
        acc = Account.objects.last()
        self.assertEqual(User.objects.filter(owner=acc).count(), 6)
        u1, u2, u3, u4, u5, u6 = User.objects.all()[n_users:]
        ...
        self.assertEqual(u5.groups.all()[0].name, 'Time-Keeping')
        self.assertEqual(u6.groups.all()[0].name, 'Supervision')
