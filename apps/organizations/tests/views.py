...
class CompanyViewTest(TestCase):
    ...
    def test_get(self):
        ...
        self.user.user_permissions.add(*ensure_permissions(Company, ['add']))
        res = self.client.get(reverse('company', kwargs={'action': 'add'}))
        self.assertEqual(res.status_code, 200)
        ...
        self.user.user_permissions.add(*ensure_permissions(Company, ['change']))
        res = self.client.get(reverse('company', kwargs={'pk': self.company.id, 'action': 'edit'}))
        self.assertEqual(res.status_code, 200)
...
class PositionViewTest(TestCase):
    ...
    def test_get(self):
        ...
        # Give permission to add, now it can
        self.user.user_permissions.add(*ensure_permissions(Position, ['add']))
        res = self.client.get(reverse('position', kwargs={'action': 'add'}))
        self.assertEqual(res.status_code, 200)
        ...
        self.user.user_permissions.add(*ensure_permissions(Position, ['change labour types']))
        res = self.client.get(reverse('position', kwargs={'pk': self.position.id, 'action': 'edit'}))
        self.assertEqual(res.status_code, 200)
...
class TeamViewTest(TestCase):
    ...
    def test_get(self):
        ...
        # Give permission to add, now it can
        self.user.user_permissions.add(*ensure_permissions(Team, ['add']))
        res = self.client.get(reverse('team', kwargs={'action': 'add'}))
        self.assertEqual(res.status_code, 200)
